...
src/
  ...
  AccountServerApi.js 账户服务端点(默认 note.wiz.cn)
...
- **私有化 endpoint 优先级**:CLI `--endpoint` > `WIZ_ENDPOINT` env > 已存 token 里绑定的 endpoint > 默认 `https://note.wiz.cn`(旧 host `as.wiz.cn` 证书在 ZeroSSL 续期间隙过期,已切到品牌域名 note.wiz.cn,同一套 AS 接口)。改动这条链路要保留原优先级。
